...
async def create_account(db: db_dependency,
...
async def refill_account(db: db_dependency, amount: AmountRequest,
...
async def withdraw_account(db: db_dependency, amount: AmountRequest,
                         account_id: int = Path(gt=0)):
    account_model = db.query(Account).filter(Account.id == account_id).first()
    if account_model is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    if account_model.blocked:
...
def tranfer_operation_by_id(db: db_dependency, from_account: int, to_account: int, amount: AmountRequest):
